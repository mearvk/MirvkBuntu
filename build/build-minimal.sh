...
lb config --distribution "$UBUNTU_SUITE" --architectures "$ARCH" --binary-images iso-hybrid --debian-installer live --apt-recommends false --archive-areas "main restricted universe multiverse" --iso-application "MirvkBuntu" --iso-publisher "MEARVK LLC" --iso-volume "$ISO_LABEL-MINIMAL" --linux-packages none
...
write_release_manifest "$OUTPUT_ROOT/MirvkBuntu-minimal-${ARCH}.iso" "$WORK_DIR" "minimal"
validate_release_iso "$OUTPUT_ROOT/MirvkBuntu-minimal-${ARCH}.iso" "$WORK_DIR" "minimal"
