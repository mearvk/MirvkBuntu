...
# GTK source acquisition for MirvkBuntu.
# Canonical build input is gnome-source/gtk/source/ (never the module root).
#
# Pinning: set GTK_REF (tag/branch/commit) to pin a version, e.g.
#   GTK_REF=<tag> bash pull-source.sh
...
SOURCE="$DEST/source"
URL="${GTK_URL:-https://github.com/GNOME/gtk.git}"
REF="${GTK_REF:-main}"
...
test -f "$TMP/$MODULE/meson.build" || { echo 'ERROR: incomplete GTK source tree' >&2; exit 1; }
...
printf 'Imported verified GTK source (ref %s -> %s) into %s\n' "$REF" "$RESOLVED" "$SOURCE"
