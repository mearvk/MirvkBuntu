...
# Mutter source acquisition for MirvkBuntu.
# Canonical build input is gnome-source/mutter/source/ (never the module root).
#
# Pinning: set MUTTER_REF (tag/branch/commit) to pin a version, e.g.
#   MUTTER_REF=<tag> bash pull-source.sh
...
SOURCE="$DEST/source"
URL="${MUTTER_URL:-https://github.com/GNOME/mutter.git}"
REF="${MUTTER_REF:-main}"
...
test -f "$TMP/$MODULE/meson.build" || { echo 'ERROR: incomplete Mutter source tree' >&2; exit 1; }
...
printf 'Imported verified Mutter source (ref %s -> %s) into %s\n' "$REF" "$RESOLVED" "$SOURCE"
