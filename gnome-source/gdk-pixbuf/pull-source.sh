...
# GDK-Pixbuf source acquisition for MirvkBuntu.
# Canonical build input is gnome-source/gdk-pixbuf/source/ (never the module root).
#
# Pinning: set GDK_PIXBUF_REF (tag/branch/commit) to pin a version, e.g.
#   GDK_PIXBUF_REF=<tag> bash pull-source.sh
...
SOURCE="$DEST/source"
URL="${GDK_PIXBUF_URL:-https://github.com/GNOME/gdk-pixbuf.git}"
REF="${GDK_PIXBUF_REF:-main}"
...
test -f "$TMP/$MODULE/meson.build" || { echo 'ERROR: incomplete GDK-Pixbuf source tree' >&2; exit 1; }
...
printf 'Imported verified GDK-Pixbuf source (ref %s -> %s) into %s\n' "$REF" "$RESOLVED" "$SOURCE"
