...
# Pango source acquisition for MirvkBuntu.
# Canonical build input is gnome-source/pango/source/ (never the module root).
#
# Pinning: set PANGO_REF (tag/branch/commit) to pin a version, e.g.
#   PANGO_REF=<tag> bash pull-source.sh
...
SOURCE="$DEST/source"
URL="${PANGO_URL:-https://github.com/GNOME/pango.git}"
REF="${PANGO_REF:-main}"
...
test -f "$TMP/$MODULE/meson.build" || { echo 'ERROR: incomplete Pango source tree' >&2; exit 1; }
...
printf 'Imported verified Pango source (ref %s -> %s) into %s\n' "$REF" "$RESOLVED" "$SOURCE"
